...
int main ( void )
{
    ...
    EIC_CallbackRegister(EIC_PIN_15, SW0_Callback, 0);
    
//    uint16_t dacOutputValue = 1023;
//    DAC_Enable();
//    DAC_DataWrite(dacOutputValue);
...
//                            dac_value = ((uint16_t) fire_alert_V1_8KHz_raw[audio_index]) << 2;
//                            DAC_DataWrite(dac_value);
//                            audio_index++;
...
//                        }
            if (sw0_pressed == false) {
                dac_value = ((uint16_t) Fire_fire_human_voice_raw[audio_index]) << 2;
                DAC_DataWrite(dac_value);
                audio_index++;
            ...
            if (sw0_pressed == 1) {
                dac_value = ((uint16_t) Alarm_1_alert_raw[audio_index]) << 2;
                DAC_DataWrite(dac_value);
                audio_index++;
            ...
            if (sw0_pressed == 2) {
                dac_value = ((uint16_t) Alarm_2_alert_raw[audio_index]) << 2;
                DAC_DataWrite(dac_value);
                audio_index++;
